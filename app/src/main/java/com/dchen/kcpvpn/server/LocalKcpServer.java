...
public class LocalKcpServer {
    ...
    private void relayDns(ServerSession session, KcpFrame frame) {
        try {
            com.dchen.kcpvpn.vpn.PacketRouter.UdpDatagram datagram =
                    com.dchen.kcpvpn.vpn.PacketRouter.parseUdpFramePayload(frame.getPayload());
    ...
    private InetAddress chooseUdpServer(com.dchen.kcpvpn.vpn.PacketRouter.UdpDatagram datagram)
            throws IOException {
        ...
        private void readLoop() {
            ...
            try {
                while (running && session.isAlive() && !closed.get()) {
                    ...
                    lastActivityTime = System.currentTimeMillis();
                    byte[] responsePayload = com.dchen.kcpvpn.vpn.PacketRouter.buildUdpFramePayload(
